<?php
// API для авторизации администраторов (PostgreSQL версия)
...
try {
    ...
    if ($method === 'POST') {
        ...
        if ($action === 'create_admin') {
            ...
            $stmt = $pdo->prepare("SELECT id FROM admin_users WHERE username = $1");
            $stmt->execute([$username]);
            if ($stmt->fetch()) {
    ...
    if ($method === 'DELETE') {
        ...
        $stmt = $pdo->prepare("SELECT username FROM admin_users WHERE id = $1");
        ...
        // Удаление
        $stmt = $pdo->prepare("DELETE FROM admin_users WHERE id = $1");
        $stmt->execute([$adminId]);
